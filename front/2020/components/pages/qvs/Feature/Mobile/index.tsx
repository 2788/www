    ...
    <div className={style.wrapper}>
      <Menu mode="inline" level={-1}>
        <SubMenu mode="inline" title="推流">
          <MenuItem><Card title="推流协议" desc="支持主流推流协议 RTMP QUIC" /></MenuItem>
        ...
        <SubMenu mode="inline" title="视频">
          ...
          <MenuItem><Card noBorder title="自定义图像处理" desc="用户可以通过回调接口，获取当前音视频数据，实现自定义音视频数据处理" /></MenuItem>
        </SubMenu>
        ...
        <SubMenu mode="inline" title="美颜">
          <MenuItem><Card noBorder title="美颜" desc="基于字节跳动人脸识别技术，美白、磨皮、锐化、大眼、瘦脸，效果更明显" /></MenuItem>
        </SubMenu>
        <SubMenu mode="inline" title="滤镜">
          <MenuItem><Card noBorder title="滤镜" desc="多款风格滤镜，满足各类场景需要" /></MenuItem>
        </SubMenu>
        <SubMenu mode="inline" title="贴纸">
          <MenuItem><Card noBorder title="贴纸" desc="上千款贴纸任意挑选，2D、3D 人脸贴纸应有尽有" /></MenuItem>
        </SubMenu>
      </Menu>
    </div>
  )
